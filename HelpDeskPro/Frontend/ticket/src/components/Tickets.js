...
const Tickets = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:3001/api/tickets', { title, description });
  ...
  const handleRemoveTicket = async (ticketId) => {
    try {
      const response = await axios.delete(`http://localhost:3001/api/tickets/${ticketId}`);
  ...
  const handleUpdateTicket = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://localhost:3001/api/tickets/${fetchedTicket._id}`, { title: updateTitle, description: updateDescription });
      if (response.status === 200) {
